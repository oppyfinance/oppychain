...
func (k Keeper) SendCoinsFromModuleToManyAccounts(ctx sdk.Context, senderModule string, recipientAddrs []sdk.AccAddress, amts []sdk.Coins) error {
	...
	// we need to ensure the module has enough balances
	totalAmt := sdk.Coins{}
	...
	for _, el := range totalAmt {
		...
		if !balance.IsGTE(el) {
			return sdkerrors.Wrapf(sdkerrors.ErrInsufficientFunds, "balance %s is smaller than %s\n", balance.String(), el.String())
		}
	}

	for i, recipientAddr := range recipientAddrs {
		err := k.bk.SendCoinsFromModuleToAccount(ctx, senderModule, recipientAddr, amts[i])
		if err != nil {
			return err
		}
	}
...
func (k Keeper) doDistributionSends(ctx sdk.Context, distrs *distributionInfo) error {
	...
	err := k.SendCoinsFromModuleToManyAccounts(
		ctx,
		types.ModuleName,
