...
var (
	...
	defaultAcctFunds  sdk.Coins = sdk.NewCoins(
		sdk.NewCoin("poppy", sdk.NewInt(100000000000000000)),
...
func (suite *KeeperTestSuite) TestCreateBalancerPool() {
	params := suite.App.SwapKeeper.GetParams(suite.Ctx)
	...
	tests := []struct {
		...
		fn: func() {
			...
			liquidity := suite.App.SwapKeeper.GetTotalLiquidity(suite.Ctx)
...
func (suite *KeeperTestSuite) TestJoinPoolNoSwap() {
	tests := []struct {
	...
	}{
		{
			fn: func(poolId uint64) {
				...
				suite.Require().NoError(err)
				suite.Require().Equal(types.OneShare.MulRaw(50).String(), suite.App.BankKeeper.GetBalance(suite.Ctx, suite.TestAccs[1], "swap/pool/1").Amount.String())
				...
				// The pool was created with the 10000foo, 10000bar, and the pool share was minted as 100000000swap/pool/1.
				// Thus, to get the 50*OneShare swap/pool/1, (10000foo, 10000bar) * (1 / 2) balances should be provided.
	...
	for _, test := range tests {
		...
		poolId, err := suite.App.SwapKeeper.CreatePool(suite.Ctx, msg)
		suite.Require().NoError(err)
	...
	tests := []struct {
		...
		},
		{
			fn: func(poolId uint64) {
				...
				suite.Require().Equal(types.InitPoolSharesSupply.QuoRaw(2).String(), suite.App.BankKeeper.GetBalance(suite.Ctx, suite.TestAccs[0], "swap/pool/1").Amount.String())
				balancesAfter := suite.App.BankKeeper.GetAllBalances(suite.Ctx, suite.TestAccs[0])

				deltaBalances, _ := balancesBefore.SafeSub(balancesAfter)
				// The pool was created with the 10000foo, 10000bar, and the pool share was minted as 100*OneShare swap/pool/1.
				// Thus, to refund the 50*OneShare swap/pool/1, (10000foo, 10000bar) * (1 / 2) balances should be refunded.
				suite.Require().Equal("-5000", deltaBalances.AmountOf("foo").String())
				suite.Require().Equal("-5000", deltaBalances.AmountOf("bar").String())
			},
		},
...
func (suite *KeeperTestSuite) TestJoinPoolExitPool_InverseRelationship() {
	...
	for _, tc := range testCases {
		...
		poolId, err := suite.App.SwapKeeper.CreatePool(suite.Ctx, tc.pool)
		suite.Require().NoError(err)

		balanceBeforeJoin := suite.App.BankKeeper.GetAllBalances(suite.Ctx, joinPoolAcc)
		fmt.Println(balanceBeforeJoin.String())

		err = suite.App.SwapKeeper.JoinPoolNoSwap(suite.Ctx, joinPoolAcc, poolId, tc.joinPoolShareAmt, sdk.Coins{})
		suite.Require().NoError(err)
...
func (suite *KeeperTestSuite) TestActiveBalancerPool() {
	...
	for _, tc := range testCases {
		...
		for _, acc := range suite.TestAccs {
			...
			err := suite.App.SwapKeeper.JoinPoolNoSwap(suite.Ctx, suite.TestAccs[0], poolId, types.OneShare.MulRaw(50), sdk.Coins{})
			suite.Require().NoError(err)
			_, err = suite.App.SwapKeeper.ExitPool(suite.Ctx, suite.TestAccs[0], poolId, types.InitPoolSharesSupply.QuoRaw(2), sdk.Coins{})
			suite.Require().NoError(err)
			...
			if tc.expectPass {
				_, err = suite.App.SwapKeeper.JoinSwapExactAmountIn(suite.Ctx, suite.TestAccs[0], poolId, foocoins, sdk.ZeroInt())
				suite.Require().NoError(err)
				_, err = suite.App.SwapKeeper.JoinSwapShareAmountOut(suite.Ctx, suite.TestAccs[0], poolId, "foo", types.OneShare.MulRaw(10), sdk.NewInt(1000000000000000000))
				suite.Require().NoError(err)
				_, err = suite.App.SwapKeeper.ExitSwapShareAmountIn(suite.Ctx, suite.TestAccs[0], poolId, "foo", types.OneShare.MulRaw(10), sdk.ZeroInt())
				suite.Require().NoError(err)
				_, err = suite.App.SwapKeeper.ExitSwapExactAmountOut(suite.Ctx, suite.TestAccs[0], poolId, foocoin, sdk.NewInt(1000000000000000000))
				suite.Require().NoError(err)
			} else {
				suite.Require().Error(err)
				_, err = suite.App.SwapKeeper.JoinSwapShareAmountOut(suite.Ctx, suite.TestAccs[0], poolId, "foo", types.OneShare.MulRaw(10), sdk.NewInt(1000000000000000000))
...
func (suite *KeeperTestSuite) TestJoinSwapExactAmountInConsistency() {
	...
	for _, tc := range testCases {
		...
		suite.Run(tc.name, func() {
			...
			shares, err := suite.App.SwapKeeper.JoinSwapExactAmountIn(ctx, suite.TestAccs[0], poolID, tc.tokensIn, tc.shareOutMinAmount)
			suite.Require().NoError(err)
			suite.Require().Equal(tc.expectedSharesOut, shares)

			tokenOutAmt, err := suite.App.SwapKeeper.ExitSwapShareAmountIn(
				ctx,
				suite.TestAccs[0],
				poolID,
				tc.tokensIn[0].Denom,
...
// 	err = suite.App.SwapKeeper.SetStableSwapScalingFactors(suite.Ctx, testScalingFactors, poolID, "")
// 	suite.Require().NoError(err)

// 	poolI, err := suite.App.SwapKeeper.GetPoolAndPoke(suite.Ctx, poolID)
// 	suite.Require().NoError(err)
