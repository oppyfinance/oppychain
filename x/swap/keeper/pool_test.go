...
func (suite *KeeperTestSuite) TestCleanupPool() {
	...
	for _, acc := range []sdk.AccAddress{acc1, acc2, acc3} {
		err := simapp.FundAccount(
			...
			acc,
			sdk.NewCoins(
				sdk.NewCoin("ujolt", sdk.NewInt(1000000000)),
	...
	poolId, err := suite.app.SwapKeeper.CreateBalancerPool(suite.ctx, acc1, defaultPoolParams, []types.PoolAsset{
		{
	...
	for _, acc := range []sdk.AccAddress{acc2, acc3} {
		err = suite.app.SwapKeeper.JoinPool(suite.ctx, acc, poolId, types.OneShare.MulRaw(100), sdk.NewCoins(
			sdk.NewCoin("foo", sdk.NewInt(1000)),
			sdk.NewCoin("bar", sdk.NewInt(1000)),
			sdk.NewCoin("baz", sdk.NewInt(1000)),
		))
	...
	err = suite.app.SwapKeeper.CleanupBalancerPool(suite.ctx, []uint64{poolId}, []string{})
	suite.NoError(err)
	for _, acc := range []sdk.AccAddress{acc1, acc2, acc3} {
...
func (suite *KeeperTestSuite) TestCleanupPoolRandomized() {
	...
	for _, acc := range []sdk.AccAddress{acc1, acc2, acc3} {
		...
		coins = append(coins, sdk.NewCoin("ujolt", sdk.NewInt(1000000000)))
	...
	poolId, err := suite.app.SwapKeeper.CreateBalancerPool(suite.ctx, acc1, defaultPoolParams, initialAssets, "")
	suite.NoError(err)

	for _, acc := range []sdk.AccAddress{acc2, acc3} {
		err = suite.app.SwapKeeper.JoinPool(suite.ctx, acc, poolId, types.OneShare, coinOf[acc.String()])
		suite.NoError(err)
	}
...
func (suite *KeeperTestSuite) TestCleanupPoolErrorOnSwap() {
	...
	_, _, err = suite.app.SwapKeeper.SwapExactAmountIn(suite.ctx, acc1, poolId, sdk.NewCoin("foo", sdk.NewInt(1)), "bar", sdk.NewInt(1))
	suite.Error(err)
}
