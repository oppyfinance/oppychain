...
func (suite *KeeperTestSuite) TestActiveBalancerPoolSwap() {
	...
	for _, tc := range testCases {
		...
		for _, acc := range suite.TestAccs {
			...
			if tc.expectPass {
				_, err := suite.App.SwapKeeper.SwapExactAmountIn(suite.Ctx, suite.TestAccs[0], poolId, foocoin, "bar", sdk.ZeroInt())
				suite.Require().NoError(err)
				_, err = suite.App.SwapKeeper.SwapExactAmountOut(suite.Ctx, suite.TestAccs[0], poolId, "bar", sdk.NewInt(1000000000000000000), foocoin)
				suite.Require().NoError(err)
			} else {
				_, err := suite.App.SwapKeeper.SwapExactAmountIn(suite.Ctx, suite.TestAccs[0], poolId, foocoin, "bar", sdk.ZeroInt())
