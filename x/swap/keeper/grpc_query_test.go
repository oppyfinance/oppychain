...
func (suite *KeeperTestSuite) TestQueryTotalShares() {
	...
	suite.Require().NoError(err)
	err = suite.app.SwapKeeper.MintPoolShareToAccount(suite.ctx, pool, acc1, types.OneShare.MulRaw(10))
	suite.Require().NoError(err)
	suite.Require().NoError(suite.app.SwapKeeper.SetPool(suite.ctx, pool))
