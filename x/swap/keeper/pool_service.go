...
// the form of swap/pool/{poolID}. In addition, the x/bank metadata is updated
// to reflect the newly created swap share denomination.
func (k Keeper) CreatePool(ctx sdk.Context, msg types.CreatePoolMsg) (uint64, error) {
	...
	poolShareDisplayDenom := fmt.Sprintf("swap-%d", pool.GetId())
	k.bankKeeper.SetDenomMetaData(ctx, banktypes.Metadata{
		Description: fmt.Sprintf("The share token of the swap pool %d", pool.GetId()),
