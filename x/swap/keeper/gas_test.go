...
func (suite *KeeperTestSuite) measureJoinPoolGas(
	...
	err := suite.App.SwapKeeper.JoinPoolNoSwap(suite.Ctx, addr, poolID, shareOutAmountMax, maxCoins)
...
func (suite *KeeperTestSuite) TestJoinPoolGas() {
	...
	suite.FundAcc(defaultAddr, sdk.NewCoins(
		sdk.NewCoin("poppy", sdk.NewInt(10000000000000)),
	...
	for i := 1; i < startAveragingAt; i++ {
		err := suite.App.SwapKeeper.JoinPoolNoSwap(suite.Ctx, defaultAddr, poolId, minShareOutAmount, sdk.Coins{})
		suite.Require().NoError(err)
	}
...
func (suite *KeeperTestSuite) TestRepeatedJoinPoolDistinctDenom() {
	...
	suite.FundAcc(defaultAddr, sdk.NewCoins(
		sdk.NewCoin("poppy", sdk.NewInt(1000000000000000000)),
	))
	...
	for i := 1; i <= denomNumber; i++ {
		...
		_, err := suite.App.SwapKeeper.CreatePool(suite.Ctx, msg)
		suite.Require().NoError(err)
	}
	...
	for i := 2; i < denomNumber; i++ {
		err := suite.App.SwapKeeper.JoinPoolNoSwap(suite.Ctx, defaultAddr, uint64(i), minShareOutAmount, sdk.Coins{})
		suite.Require().NoError(err)
	}
