...
import (
	...
	balanacertypes "gitlab.com/oppy-finance/oppychain/x/swap/pool_models/balancer"
...
func (suite *KeeperTestSuite) measureJoinPoolGas(
	...
	err := suite.app.SwapKeeper.JoinPool(suite.ctx, addr, poolID, shareOutAmountMax, maxCoins)
...
func (suite *KeeperTestSuite) TestJoinPoolGas() {
	...
	err := simapp.FundAccount(suite.app.BankKeeper, suite.ctx, defaultAddr, sdk.NewCoins(
		sdk.NewCoin("ujolt", sdk.NewInt(10000000000000)),
	...
	for i := 1; i < startAveragingAt; i++ {
		err := suite.app.SwapKeeper.JoinPool(suite.ctx, defaultAddr, poolId, minShareOutAmount, sdk.Coins{})
		suite.Require().NoError(err)
	}
...
func (suite *KeeperTestSuite) TestRepeatedJoinPoolDistinctDenom() {
	...
	err := simapp.FundAccount(suite.app.BankKeeper, suite.ctx, defaultAddr, sdk.NewCoins(
		sdk.NewCoin("ujolt", sdk.NewInt(1000000000000000000)),
	))
	suite.Require().NoError(err)
	...
	for i := 1; i <= denomNumber; i++ {
		...
		_, err = suite.app.SwapKeeper.CreateBalancerPool(suite.ctx, defaultAddr, defaultPoolParams, poolAssets, "")
		suite.Require().NoError(err)
	}
	...
	for i := 2; i < denomNumber; i++ {
		err := suite.app.SwapKeeper.JoinPool(suite.ctx, defaultAddr, uint64(i), minShareOutAmount, sdk.Coins{})
		suite.Require().NoError(err)
	}
