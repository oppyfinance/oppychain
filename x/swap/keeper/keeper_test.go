...
func (suite *KeeperTestSuite) SetupTest() {
	...
	types.RegisterQueryServer(queryHelper, suite.app.SwapKeeper)
...
func (suite *KeeperTestSuite) prepareBalancerPoolWithPoolParams(PoolParams balancer.PoolParams) uint64 {
	...
	for _, acc := range []sdk.AccAddress{acc1, acc2, acc3} {
		err := simulation.FundAccount(suite.app.BankKeeper, suite.ctx, acc, sdk.NewCoins(
	...
	poolId, err := suite.app.SwapKeeper.CreateBalancerPool(suite.ctx, acc1, PoolParams, []types.PoolAsset{
		{
...
func (suite *KeeperTestSuite) prepareBalancerPool() uint64 {
	...
	spotPrice, err := suite.app.SwapKeeper.CalculateSpotPriceWithSwapFee(suite.ctx, poolId, "foo", "bar")
	suite.NoError(err)
	suite.Equal(sdk.NewDec(2).String(), spotPrice.String())
	spotPrice, err = suite.app.SwapKeeper.CalculateSpotPriceWithSwapFee(suite.ctx, poolId, "bar", "baz")
	suite.NoError(err)
	suite.Equal(sdk.NewDecWithPrec(15, 1).String(), spotPrice.String())
	spotPrice, err = suite.app.SwapKeeper.CalculateSpotPriceWithSwapFee(suite.ctx, poolId, "baz", "foo")
	suite.NoError(err)
