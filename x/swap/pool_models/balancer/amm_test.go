...
func (suite *KeeperTestSuite) TestBalancerSpotPrice() {
	...
	for _, tc := range tests {
		...
		pool, err := suite.App.SwapKeeper.GetPoolAndPoke(suite.Ctx, poolId)
	...
	for _, tc := range testcases {
		for _, swapFee := range swapFeeCases {
			t.Run(getTestCaseName(tc, swapFee), func(t *testing.T) {
				...
				tol := sdk.NewDec(1)
				require.True(tools.DecApproxEq(t, expected, actual, tol))
			})
		}
	}
}
	...
	for _, tc := range testcases {
		for _, swapFee := range swapFeeCases {
			t.Run(getTestCaseName(tc, swapFee), func(t *testing.T) {
				...
				tol := sdk.NewDec(1)
				require.True(tools.DecApproxEq(t, initialCalcTokenOut.ToDec(), inverseCalcTokenOut, tol))
			})
		}
	}
}
