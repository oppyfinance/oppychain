...
import (
	...
	"gitlab.com/oppy-finance/oppychain/x/swap/pool_models/internal/cfmm_common"
...
const (
	...
	// here, we need to have the new algo from the first block
	v10Fork = 0
)
...
func solveConstantFunctionInvariant(
	...
	yToWeightRatio := math.Pow(y, weightRatio)
...
func (p Pool) SpotPrice(ctx sdk.Context, baseAsset, quoteAsset string) (sdk.Dec, error) {
	...
	ratio := math.SigFigRound(fullRatio, types.SigFigs)
	return ratio, nil
}
