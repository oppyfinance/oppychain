...
func (suite *KeeperTestSuite) TestEndblockerWithdrawAllMaturedLockups() {
	...
	suite.Require().Equal(suite.App.BankKeeper.GetAccountsBalances(suite.Ctx)[0].Address, addr1.String())
	suite.Require().Equal(suite.App.BankKeeper.GetAccountsBalances(suite.Ctx)[0].Coins, totalCoins)
