...
func (s *KeeperTestHelper) Setup() {
	...
	app, ok := simapp.New(tempPath).(*app.App)
	if !ok {
		panic("fail to create the test app")
	}
	s.App = app
	s.Ctx = s.App.BaseApp.NewContext(false, tmproto.Header{Height: 1, ChainID: "oppyChain-1", Time: time.Now().UTC()})
...
//
//	poolCreationFee := s.App.SwapKeeper.GetParams(s.Ctx)
//	s.FundAcc(acc1, poolCreationFee.PoolCreationFee)
//
//	pools := []swaptypes.PoolI{}
//	for index, multiplier := range multipliers {
//		token := fmt.Sprintf("token%d", index)
//		uosmoAmount := swaptypes.InitPoolSharesSupply.ToDec().Mul(multiplier).RoundInt()
//
...
//
//		poolId, err := s.App.SwapKeeper.CreatePool(s.Ctx, msg)
//		s.Require().NoError(err)
//
//		pool, err := s.App.SwapKeeper.GetPoolAndPoke(s.Ctx, poolId)
//		s.Require().NoError(err)
//
...
//
//	_, err := s.App.SwapKeeper.SwapExactAmountOut(
//		s.Ctx,
//		acc1,
//		poolId,
//		fromAsset.Denom,
...
//	s.Require().NoError(err)
//
//	spotPrice, err := s.App.SwapKeeper.CalculateSpotPrice(s.Ctx, poolId, toAsset.Denom, fromAsset.Denom)
//	s.Require().NoError(err)
//
//	return spotPrice
//}
