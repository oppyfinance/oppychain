...
import { EpochInfo } from "oppy-finance-oppychain-client-ts/oppyfinance.oppychain.epochs/types"
...
export default {
	...
	actions: {
		init({ dispatch, rootGetters }) {
			console.log('Vuex module: oppyfinance.oppychain.epochs initialized!')
		...
		async QueryEpochInfos({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				...
				let value= (await client.OppyfinanceOppychainEpochs.query.queryEpochInfos()).data
		...
		async QueryCurrentEpoch({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				const key = params ?? {};
				const client = initClient(rootGetters);
				let value= (await client.OppyfinanceOppychainEpochs.query.queryCurrentEpoch(query ?? undefined)).data
				
					
				while (all && (<any> value).pagination && (<any> value).pagination.next_key!=null) {
					let next_values=(await client.OppyfinanceOppychainEpochs.query.queryCurrentEpoch({...query ?? {}, 'pagination.key':(<any> value).pagination.next_key} as any)).data
